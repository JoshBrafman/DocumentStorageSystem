...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    public Value get(Key key)
    {
        ...
        if(entry != null)
        {
            if (entry.hasUri) {//Then change the URI reference to the actual Document
                try {
                    this.deserializeAndDelete(entry);
                } catch (IOException e) {
                    return null;
                }
...
@Override
    public Value put(Key key, Value val)
    {
        ...
        //if the key already existed, replace the value
        Entry alreadyThere = this.get(this.root, key, this.height);
        if(alreadyThere != null)
        {
            Object previous = alreadyThere.val;
            if (alreadyThere.hasUri){

                try {
                    this.deserializeAndDelete(alreadyThere);
                    Document oldDoc = (Document) alreadyThere.val;
    ...
    private void deserializeAndDelete(Entry entry) throws IOException {
        Key key = (Key) entry.val;
